const fromamountEl = document.getElementById('amount')
...
const getExchangeRate = async () => {
  ...
  // console.log(fromCurrency);
      const toCurrency = toCurrencyEl.value;
       resultEl.textContent = "Feching exchange rates..."
    ...
    try {
      ...
      // console.log(exchangeRate);
        ...
        if ( typeof exchangeRate === "undefined") {
              resultEl.textContent = `Exchange rate is not avaiable for selected currency`;
              ConvertedamountEl.value= "";
        } else {
            ConvertedamountEl.value = convertedAmount.toFixed(2);
            resultEl.textContent = `1 ${fromCurrency} = ${exchangeRate} ${toCurrency}`;
        }
    } catch (error) {
      currencycontainer.innerHTML = `<h3>Error while feching exchange rates !!!`
    }
};

// Attach this function to an event, like a button click
